<!DOCTYPE html>
<html>
    <head>
        <!--[if IE 8]>
            <link HREF="/IE8.css" REL="StyleSheet" TYPE="text/css">
        <![endif]-->

        <meta name="norton-safeweb-site-verification" content="xo9qbg4a-l5zi50qameyixskmht7-p257-popwtc3b7-n1jwyvzpbfh6gyps6r-kjbhcbne6jncqc1wzzavrvsdujbxrsfi68gnz29af35abe4yrh9l9kqowla4ni0m-"/>
        <meta name="description" content="Webhosting Webhotel, Danmarks bedste webhotel. Vi tilbyder kvalitets webhoteller og domæner til lave priser."/>
        <meta name="keywords" content="Webhosting, webhotel, webhoteller, domæner, WordPress, Joomla, Magento"/>
        <meta name="robots" content="index,follow"/>
        <meta name="GOGGLEBOT" content="index,follow"/>
        <meta name="verify-v1" content="h+NnTjzEHM+4ipnqh7kNa9Neg3zvHIbli1XRskGmdYU=" />
        <meta name="revisit" content="4 days"/>
        <meta name="Copyright" content="Webmaster"/>
        <meta name="Language" content="Danish, English"/>
        <meta name="distribution" content="global"/>
        <meta name="copyright" content="webhosting.dk" />
        <meta name="rating" content="General"/>
        <meta name="robots" content="ALL"/>
        <meta name="distribution" content="global"/>
        <meta name="author" content="webhosting.dk"/>
        <meta name="DC.title" content="WebHosting A/S - Webhotel, billigt, godt og effektivt"/>
        <meta name="DC.description" content="webhotel, domain, domæne,webhosting, billigt, gratis, lavpris, autoresponder, email, imap, pop3, smtp, server, virtual, registrering, registration, com,org,net,info,dk,internet,mailhotel,php,php4,asp,frontpage, drupal,magento"/>
        <meta name="DC.subject" content="Lavpris domains, webhosting siden 1996, Joomla 1 klik installation, bedste service"/>
        <meta name="DC.coverage.placeName" content="Danmark"/>
        <meta name="DC.language" content="da"/>

        <link rel="stylesheet" type="text/css" href="css/reset.css" />
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
		<link rel="stylesheet" type="text/css" href="css/style.css" />
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" crossorigin="anonymous"></script>

        <title>Webhosting Webhotel | Webhosting.dk</title>
    </head>
    <body>
        <?php include_once("header.php");?>
        
        <section id="sctn_aboutus">
            <div class="row">
                <div class="col-md-10 col-md-offset-1">
                    <div class="info-box">
                        <h2>Why host with us?</h2>
                        <p>We provide numerous features not found elsewhere</p>
                        <ul>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Multiple PHP versions
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Advanced spam filtering
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Multiple backup solutions
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Excellent customer service
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Free DNS manager and 301 redirect
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; 1 click installers for WordPress and Joomla
                            </li>
                            <li>
                                <i class="glyphicon glyphicon-ok"></i> &nbsp; Very fast servers, try our performance plugin
                            </li>
                        </ul>
                        <p>
                            To view our price list please click this <a href="#products">link</a>.
                        </p>
                    </div>
                    <div class="info-box-absolute">
                        <h2>Who are we?</h2>
                        <p>
                            We are one of the oldest hosting providers in the
                            world, we have the experience, fast servers and high
                            uptime.
                        </p>
                        <p>
                            Check our products by clicking the button
                        </p>
                        <a href="#products" class="btn btn-warning btn-lg">Products</a>
                    </div>
                </div>
            </div>
        </section>
        <section id="products">
            <div class="pricing-tables attached">
                <div class="row ">
                    <div class="col-sm-12 col-md-2 col-lg-2 col-md-offset-1 col-lg-offset-1">
                        <div class="plan first">
                            <div class="head">
                                <h2>Domain</h2>
                            </div>
                            <ul class="item-list">
                                <li>
                                    <strong>Lowest prices</strong></strong>
                                </li>
                                <li>
                                    <strong>FREE DNS hosting</strong></li>
                                <li>
                                    <strong>Many different types</strong></li>
                                <li>
                                    <strong>Advanced DNS manager</strong></li>
                                <li>
                                    <strong>Easy upgrade</strong></li>
                            </ul>
                            <div class="price mediafix">
                                <h4>From only</h4>
                                <h3>44 <span>DKK</span></h3>
                                <h4>for a .DK domain</h4>
                            </div>
                            <a class="btn-clean btn-pad" href="index.html#contact">Order Now <i class="ion-ios-unlocked-outline"></i></a>
                        </div>
                    </div>
        
                    <div class="col-sm-12 col-md-2 col-lg-2">
                        <div class="plan first recommended">
                            <div class="head">
                                <h2>PHP Basic</h2>
                            </div>
                            <ul class="item-list">
                                <li>
                                    <strong>5 Gb web space</strong>
                                </li>
                                <li>
                                    <strong>10 Gb space for e-mail</strong></li>
                                <li>
                                    <strong>1 click WordPress, Joomla</strong></li>
                                <li>
                                    <strong>All PHP/MySQL versions</strong></li>
                                <li>
                                    <strong>256 Mb PHP memory limit</strong></li>
                            </ul>
                            <div class="price">
                                <h4>From under</h4>
                                <h3>9.04 <span>DKK</span></h3>
                                <h4>per month</h4>
                            </div>
                            <a class="btn-clean btn-pad" href="http://www.webhosting.dk/ENG/createoffernewadddomain.php?orderphpbasic=J&tdomain=x&newsystemfilter=&newbackupmaster=&redel=&newpopaccounts=&newmaxalias=&newsuperpack=&urlforwarderpro=">Order Now <i class="ion-ios-unlocked-outline"></i></a>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-2 col-lg-2">
                        <div class="plan recommended">
                            <div class="head">
                                <h2>PHP Medium</h2>
                            </div>
                            <ul class="item-list">
                                <li>
                                    <strong>10 Gb web space</strong>
                                </li>
                                <li>
                                    <strong>30 Gb space for e-mail</strong></li>
                                <li>
                                    <strong>1 click WordPress, Joomla</strong></li>
                                <li>
                                    <strong>All PHP/MySQL versions</strong></li>
                                <li>
                                    <strong>512 Mb PHP memory limit</strong></li>
                            </ul>
                            <div class="price">
                                <h4>From under</h4>
                                <h3>24.98 <span>DKK</span></h3>
                                <h4>per month</h4>
                            </div>
                            <a class="btn-clean btn-pad" href="http://www.webhosting.dk/ENG/createoffernewadddomain.php?orderphplite=J&newhtpasswd=&newssl=&tdomain=x&newsystemfilter=&newbackupmaster=&redel=&newpopaccounts=&newmaxalias=&newsuperpack=&urlforwarderpro=">Order Now <i class="ion-ios-unlocked-outline"></i></a>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-2 col-lg-2">
                        <div class="plan last recommended">
                            <div class="head">
                                <h2>PHP Business</h2>
                            </div>
                            <ul class="item-list">
                                <li>
                                    <strong>20 Gb web space</strong>
                                </li>
                                <li>
                                    <strong>60 Gb space for e-mail</strong></li>
                                <li>
                                    <strong>1 click WordPress, Joomla</strong></li>
                                <li>
                                    <strong>All PHP/MySQL versions</strong></li>
                                <li>
                                    <strong>512 Mb PHP memory limit</strong></li>
                            </ul>
                            <div class="price">
                                <h4>From under</h4>
                                <h3>49.88 <span>DKK</span></h3>
                                <h4>per month</h4>
                            </div>
                            <a class="btn-clean btn-pad" href="http://www.webhosting.dk/ENG/createoffernewadddomain.php?orderphplite=N&newhtpasswd=&newssl=&tdomain=x&newsystemfilter=J&ordergrundprisphp=J&newmysql=J&newbackupmaster=&redel=&newpopaccounts=&newmaxalias=&newsuperpack=&urlforwarderpro=">Order Now <i class="ion-ios-unlocked-outline"></i></a>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-2 col-lg-2">
                        <div class="plan last">
                            <div class="head value">
                                <h2>Mail Hosting</h2>
                            </div>
                            <ul class="item-list">
                                <li>
                                    <strong>30 Gb mail space</strong>
                                </li>
                                <li>
                                    <strong>10 IMAP/POP3 accounts</strong></li>
                                <li>
                                    <strong>50 e-mail addresses</strong></li>
                                <li>
                                    <strong>Advanced webmail</strong></li>
                                <li>
                                    <strong>FREE URL forwarder</strong></li>
                            </ul>
                            <div class="price">
                                <h4>From under</h4>
                                <h3>10 <span>DKK</span></h3>
                                <h4>per month</h4>
                            </div>
                            <a class="btn-clean btn-pad" href="index.html#contact">Order Now <i class="ion-ios-unlocked-outline"></i></a>
                        </div>
                    </div>
                </div>
                <!-- row-->
            </div>
            <!-- pricing-tables -->
            <div class="row">
                <div class="col-lg-12 text-center">
                    <a href="http://www.webhosting.dk/ENG/productmatrix.php?language=ENG" class="btn btn-warning btn-lg">Compare products</a>
                </div>
            </div>
        </section>
        
        <?php include_once("footer.php");?>
    </body>
</html>
